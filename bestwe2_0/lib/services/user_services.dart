...
class UserServices {
  ...
  Options _formOptions({Map<String, String>? extraHeaders}) => Options(
    contentType: 'multipart/form-data', // Correct content type for file/image upload
    headers: {
      ...?extraHeaders,
    },
  );
  ...
  // Future<Map<String, dynamic>> updatedata<T>({required String url, required T model, required Map<String, dynamic> Function(T) toJson, bool wrapInData = false, String method = 'POST', File? imagefile,
  // }) async {
  //   try {
  //     final Map<String, dynamic> payload = wrapInData ? {'data': toJson(model)} : toJson(model);
  ...
  //     return response.data; // ✅ return response for Flutter to use
  //   } catch (e) {
  //     print('Update error at [$url]: $e');
  //     throw e;
  //   }
  // }
  Future<Map<String, dynamic>> updatedata<T>({
    required String url,
    required T model,
    ...
    try {
      final prefs = await SharedPreferences.getInstance(); // ✅ Get token
