...
class AboutUsScreen extends StatelessWidget {
  ...
  List<String> _splitHtmlToBulletPoints(String htmlContent) {
    final text = htmlContent.replaceAll(RegExp(r'<[^>]*>'), '');
    return text.split('. ').where((point) => point.trim().isNotEmpty).toList();
  }
                ...
                child: Container(
                  ...
                  child:  Consumer<ApplicationState>(
                    builder: (context, appState, _) {
                      final about = appState.staticContentState.aboutus;
                      if (about == null) {
                        return const Center(child: CircularProgressIndicator());
                      }
                      final aboutContent = about.content;
                      // Split into points if needed
                      final bulletPoints = _splitHtmlToBulletPoints(aboutContent);
